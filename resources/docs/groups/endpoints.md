...
```bash
curl -X GET \
    -G "https://onthego2africa.com/api/sanctum/csrf-cookie" \
...
```javascript
const url = new URL(
    "https://onthego2africa.com/api/sanctum/csrf-cookie"
);
...
```bash
curl -X GET \
    -G "https://onthego2africa.com/api/" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
